...
const ViewDetails = () => {
  ...
  const completedToday = userCompletions.some(
  ...
  const completedDays = userCompletions.filter((d) =>
    last30Days.includes(d.date)
  ).length;
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 md:px-12 py-12 bg-linear-to-r from-blue-100 via-purple-100 to-green-100">
      <button
        onClick={() => navigate(-1)}
        className="btn btn-outline btn-primary btn-xs md:btn-sm mb-6"
      >
        ← Back
      </button>
      ...
      >
        ...
        <div className="md:w-1/2 h-64 md:h-auto">
          <img
            src={habit.imageUrl || "https://via.placeholder.com/600x400"}
            alt={habit.title}
          ...
          <div>
            ...
            <div className="mb-3">
              <div className="w-full bg-gray-200 h-3 rounded-full">
                <div
                  className="h-3 bg-secondary rounded-full transition-all duration-500"
            ...
            {/* Streak Badge */}
            <span className="bg-orange-500 text-white md:px-3 md:py-2 rounded-full text-sm font-medium">
              🔥 {streak} Day Streak
            </span>
          </div>
